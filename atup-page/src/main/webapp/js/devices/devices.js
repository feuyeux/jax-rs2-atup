...
function renderGetAll(data) {
	...
	if (list == null || list.length == 0) {
		...
		jQuery("#devicesDiv").html(
				"<div align='left'>" + SPAN_BEGIN + "Device ID</span>" + SPAN_BEGIN2 + "Device Name</span>" + SPAN_BEGIN2 + "Device IP</span>" + SPAN_BEGIN
						+ "Status</span>" + SPAN_BEGIN2 + "Last Updated</span></div>");
		jQuery.each(list, function(i, device) {
			var line = "<div align='left'>" + SPAN_BEGIN + device.deviceId + "</span>" + SPAN_BEGIN2 + device.deviceName + "</span>" + SPAN_BEGIN2 + device.deviceHost
