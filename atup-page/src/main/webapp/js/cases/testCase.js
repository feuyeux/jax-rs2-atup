function initial() {
    checkSignIn();
    restGet(ATUP_CASE_URI + TEST_SUITE_PATH + "/suites?start=0&size=100", GET_METHOD, renderSuiteList0);
    jQuery("#caseName").change(function () {
        restGet(ATUP_CASE_URI + TEST_SUITE_PATH + "/suites?start=0&size=100", GET_METHOD, renderSuiteList);
        var caseName = jQuery("#caseName").val();
        restGet(ATUP_CASE_URI + TEST_CASE_PATH + "/" + caseName, GET_METHOD, fresh4Update);
    });
}
